...
class Contour2D(YAMLObjectBase):
    ...
    def __repr__(self):
        ...
        """
        return f"{self.__class__.__name__}(name={self.name!r}, points={self.points!r})"
...
def create_rectangle(x: float, y: float, dx: float, dy: float, fillet: int = 0) -> Contour2D:
    """
...
def create_angularslit(
    ...
    if fillet > 0:
        ...
        r_ = dx / 2.0
        for i in range(fillet):
            _x = xc + r_ * cos(pi + i * theta_)
            _y = yc + r_ * sin(pi + i * theta_)
            points.append([_x, _y])
