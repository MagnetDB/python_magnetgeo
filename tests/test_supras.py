...
class TestSuprasEdgeCases:
    """
    Test edge cases and special scenarios for Supras
    """
    
    def test_negative_bore_values(self):
        """Test Supras with negative bore values"""
        supras = Supras(
            name="negative_bore",
            magnets=[],
            innerbore=-5.0,
        ...
        assert supras.outerbore == -1.0
    
    def test_inverted_bore_values(self):
        """Test Supras with inverted bore values (inner > outer)"""
        supras = Supras(
            name="inverted_bore",
            magnets=[],
            innerbore=10.0,
            outerbore=5.0  # Smaller than inner
        )
        assert supras.innerbore == 10.0
        assert supras.outerbore == 5.0
    
    def test_very_large_values(self):
        """Test Supras with very large values"""
        large_val = 1e12
        supras = Supras(
            name="large_values",
            magnets=[],
            innerbore=large_val,
            outerbore=large_val * 2
        )
        assert supras.innerbore == large_val
        assert supras.outerbore == large_val * 2
    
    def test_unicode_name(self):
        """Test Supras with Unicode name"""
        unicode_name = "超导磁体_тест_🧲"
        supras = Supras(
            name=unicode_name,
            magnets=[],
            innerbore=5.0,
        ...
        assert supras.name == unicode_name
    
    def test_special_characters_name(self):
        """Test Supras with special characters in name"""
        special_name = "supras!@#$%^&*()_+-=[]{}|;':\",./<>?"
        supras = Supras(
            name=special_name,
            magnets=[],
            innerbore=1.0,
        ...
        assert supras.name == special_name
    
    def test_mixed_magnet_types(self):
        """Test Supras with mixed magnet types (objects and strings)"""
        supra_obj = Supra(name="object", r=[1.0, 2.0], z=[0.0, 1.0], n=1, struct="")
        mixed_magnets = [supra_obj, "string_magnet", supra_obj]
        
        supras = Supras(
            name="mixed_types",
            magnets=mixed_magnets,
            innerbore=0.5,
            outerbore=2.5
        )
        assert supras.magnets == mixed_magnets
    
    def test_zero_bore_values(self):
        """Test Supras with zero bore values"""
        supras = Supras(
            name="zero_bore",
            magnets=[],
            innerbore=0.0,
        ...
        assert supras.innerbore == 0.0
        assert supras.outerbore == 0.0
