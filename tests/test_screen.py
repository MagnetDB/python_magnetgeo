...
class TestScreenEdgeCases:
    """
    Test edge cases and special scenarios for Screen
    """
    
    def test_zero_dimensions(self):
        """Test Screen with zero dimensions"""
        zero_r_screen = Screen(name="zero_r", r=[5.0, 5.0], z=[0.0, 10.0])
        assert zero_r_screen.get_lc() == 0.0  # (5.0 - 5.0) / 10.0 = 0.0
        
        zero_z_screen = Screen(name="zero_z", r=[0.0, 10.0], z=[3.0, 3.0])
        assert zero_z_screen.get_lc() == 1.0  # (10.0 - 0.0) / 10.0 = 1.0
    
    def test_negative_coordinates(self):
        """Test Screen with negative coordinates"""
        ...
        assert negative_screen.r == [-10.0, -5.0]
        assert negative_screen.z == [-20.0, -10.0]
        assert negative_screen.get_lc() == 0.5  # (-5.0 - (-10.0)) / 10.0 = 0.5
    
    def test_inverted_coordinates(self):
        """Test Screen with inverted coordinates (r[0] > r[1])"""
        inverted_screen = Screen(
            name="inverted_test",
            r=[20.0, 10.0],  # r[0] > r[1]
            z=[30.0, 20.0]   # z[0] > z[1]
        )
        
        assert inverted_screen.r == [20.0, 10.0]
        assert inverted_screen.z == [30.0, 20.0]
        assert inverted_screen.get_lc() == -1.0  # (10.0 - 20.0) / 10.0 = -1.0
    
    def test_very_large_coordinates(self):
        """Test Screen with very large coordinates"""
        large_val = 1e12
        large_screen = Screen(
            name="large_test",
            r=[large_val, large_val * 2],
            z=[large_val * 3, large_val * 4]
        )
        
        assert large_screen.r == [large_val, large_val * 2]
        assert large_screen.z == [large_val * 3, large_val * 4]
        assert large_screen.get_lc() == large_val / 10.0
    
    def test_very_small_coordinates(self):
        """Test Screen with very small coordinates"""
        small_val = 1e-12
        small_screen = Screen(
            name="small_test",
            r=[small_val, small_val * 2],
            z=[small_val * 3, small_val * 4]
        )
        
        assert small_screen.r == [small_val, small_val * 2]
        assert small_screen.z == [small_val * 3, small_val * 4]
        assert abs(small_screen.get_lc() - small_val / 10.0) < 1e-20
    
    def test_unicode_name(self):
        """Test Screen with Unicode name"""
        unicode_name = "屏幕_экран_🛡️"
        unicode_screen = Screen(
            name=unicode_name,
            r=[1.0, 2.0],
            z=[0.0, 1.0]
        )
        assert unicode_screen.name == unicode_name
    
    def test_special_characters_name(self):
        """Test Screen with special characters in name"""
        special_name = "screen!@#$%^&*()_+-=[]{}|;':\",./<>?"
        special_screen = Screen(
            name=special_name,
            r=[1.0, 2.0],
            z=[0.0, 1.0]
        )
        assert special_screen.name == special_name
    
    def test_mixed_numeric_types(self):
        """Test Screen with mixed numeric types (int, float)"""
        mixed_screen = Screen(
            name="mixed_types",
            r=[1, 2.5],    # int, float
            z=[0.0, 3]     # float, int
        ...
        assert mixed_screen.r == [1, 2.5]
        assert mixed_screen.z == [0.0, 3]
        assert mixed_screen.get_lc() == 0.15  # (2.5 - 1) / 10.0 = 0.15
