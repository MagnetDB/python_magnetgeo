...
class TestShapeEdgeCases:
    """
    Test edge cases and special scenarios for Shape
    """
    
    def test_extreme_angle_values(self):
        """Test Shape with extreme angle values"""
        extreme_angles = [-360.0, -180.0, 0.0, 180.0, 360.0, 720.0, 1080.0]
        shape = Shape(name="extreme", profile="test", angle=extreme_angles)
        assert shape.angle == extreme_angles
    
    def test_extreme_length_values(self):
        """Test Shape with extreme length values"""
        extreme_lengths = [0.0, 0.001, 1e-10, 1e10, float('inf')]
        # Note: float('inf') might cause issues in JSON serialization
        shape = Shape(name="extreme", profile="test", length=extreme_lengths[:-1])
        assert shape.length == extreme_lengths[:-1]
    
    def test_negative_turn_numbers(self):
        """Test Shape with negative turn numbers"""
        negative_turns = [-1, -5, 0, 1, 5]
        shape = Shape(name="negative", profile="test", onturns=negative_turns)
        assert shape.onturns == negative_turns
    
    def test_very_long_lists(self):
        """Test Shape with very long parameter lists"""
        long_length = [float(i) for i in range(100)]
        long_angle = [float(i * 3.6) for i in range(100)]  # 0 to 356.4 degrees
        long_onturns = list(range(1, 101))
        ...
        shape = Shape(
            ...
            length=long_length,
            angle=long_angle,
            onturns=long_onturns
        )
        assert len(shape.length) == 100
        assert len(shape.angle) == 100
        assert len(shape.onturns) == 100
    
    def test_unicode_in_strings(self):
        """Test Shape with Unicode characters in string fields"""
        unicode_name = "形状_тест_🔺"
        unicode_profile = "轮廓_профиль_⭐"
        unicode_position = "位置_позиция_📍"
        
        shape = Shape(
            name=unicode_name,
            profile=unicode_profile,
            position=unicode_position
        )
        assert shape.name == unicode_name
        assert shape.profile == unicode_profile
        assert shape.position == unicode_position
    
    def test_special_characters_in_strings(self):
        """Test Shape with special characters in string fields"""
        special_name = "shape!@#$%^&*()_+-=[]{}|;':\",./<>?"
        special_profile = "profile\\with/special\\chars"
        special_position = "position\twith\nspecial\rchars"
        ...
        assert shape.name == special_name
        assert shape.profile == special_profile
        assert shape.position == special_position
