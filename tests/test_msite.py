...
class TestMSiteEdgeCases:
    ...
    def test_msite_empty_magnets_list(self):
        ...
        # Operations should handle empty list gracefully
        channels = msite.get_channels("empty")
        assert channels == {}
        
        names = msite.get_names("empty")
        assert names == []

    def test_msite_with_none_magnets(self):
        """Test MSite behavior with None magnets"""
        # This might raise an error depending on implementation
        try:
            msite = MSite(
                name="none_magnets_site",
                magnets=None,
                screens=None,
                z_offset=None,
                r_offset=None,
            ...
            # Expected if implementation doesn't handle None magnets
            pass

    def test_msite_with_very_large_offsets(self):
        """Test MSite with very large offset values"""
        msite = MSite(
            name="large_offsets_site",
            magnets=["magnet"],
            screens=None,
            z_offset=[1e6, 1e7],
            r_offset=[1e5, 1e6],
        ...
        assert msite.z_offset == [1e6, 1e7]
        assert msite.r_offset == [1e5, 1e6]
        assert msite.paralax == [1e3, 1e4]

    def test_msite_with_negative_offsets(self):
        """Test MSite with negative offset values"""
        msite = MSite(
            name="negative_offsets_site",
            magnets=["magnet"],
            screens=None,
            z_offset=[-10.0, -5.0],
            r_offset=[-2.0, -1.0],
        ...
        assert msite.z_offset == [-10.0, -5.0]
        assert msite.r_offset == [-2.0, -1.0]
        assert msite.paralax == [-0.5, -0.1]
