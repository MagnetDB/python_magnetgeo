...
def search(
    ...
    for yaml_file in sorted(yaml_dir.glob(pattern)):
        if "tmp" in yaml_file.parts:
            continue
        if excluded and any(part in excluded for part in yaml_file.parts):
            continue
        if "meshdata" in yaml_file.stem:
            print(f"  [skip] skipping mesdata file: {yaml_file}")
            continue
        try:
        ...
        for field_path, value in cad_refs.items():
            ...
            if normalized_query is None or normalized_query == value:
                existing = next((m for m in matches if m["cad_value"] == value), None)
                if existing:
                    print(
                        f"  [warn] duplicate CAD ref {value!r}: "
                        f"{yaml_file} (already seen in {existing['file']})",
                        flush=True,
                    )
                else:
                    matches.append(
...
def main():
    ...
    parser.add_argument(
        "--field", default=None, help="Restrict to a specific field path (e.g. model3d.cad)"
    )
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="DIR",
        help="Directory names to exclude from search (e.g. --exclude-dirs archive deprecated)",
    )
    parser.add_argument(
        "--output", default="cad_refs.csv", help="Output CSV file (default: cad_refs.csv)"
    )
    ...
    results = search(yaml_dir, args.cad_ref, args.type, args.recursive, args.exclude_dirs)
    ...
    if not results:
        msg = (
            f"No matches found for CAD ref: {args.cad_ref!r}"
            if args.cad_ref
            else "No CAD refs found."
        )
        print(msg)
        sys.exit(0)

    title = (
        f"Found {len(results)} match(es) for {args.cad_ref!r}"
        if args.cad_ref
        else f"Found {len(results)} CAD ref(s)"
    )
